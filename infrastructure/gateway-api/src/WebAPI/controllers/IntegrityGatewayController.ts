...
export class IntegrityGatewayController {
    ...
    private async getAllHashes(req: Request, res: Response): Promise<void> {
        try {
            const hashes = await this.gatewayService.getAllHashes();
            res.status(200).json(hashes);
        } catch (err) {
            const message = (err as Error).message;
            await this.loggerService.log(`Gateway Integrity Error (GetAllHashes): ${message}`);
            res.status(500).json({ message });
        }
    }
